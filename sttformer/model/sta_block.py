...
from .Muti import MultiScale_TemporalConv
...
class STA_Block(nn.Module):
    def __init__(self, in_channels, out_channels, qkv_dim,
        ...
        # self.out_nett = MultiScale_TemporalConv(out_channels, out_channels, kernel_size=kernel_size[0], stride=1, dilations=[1, 2], residual=False)
    ...
    def forward(self, x):
        ...
        attention = F.softmax(torch.einsum('nhctu,nhctv->nhuv', [q, k]) / (self.qkv_dim * T), -1) * self.alphas
