    ...
    for i in tqdm(range(N)):
        for p in range(M):
            ...
            # if np.all((sample[:,0,:] == 0)):
                # continue
            d = sample[:,0,1:2]
            a, b, c, d = sample[:,0,5], sample[:,0,6], sample[:,0,11], sample[:,0,12]
            a_b_c_d = (a + b + c + d) / 4
            c_d = (c + d) / 2
            v1 = a_b_c_d-c_d
...
def create_aligned_mmvrac_dataset(file_list=['data/ntu/MMVRAC_CSv1.npz', 'data/ntu/MMVRAC_CSv2.npz']):
    for file in file_list:
        ...
        for split in splits:
            data = org_data[split]
            N, T, _ = data.shape
            data = data.reshape((N, T, 2, 17, 3)).transpose(0, 4, 1, 3, 2)
            aligned_data = align_UAV_skeleton(data)
